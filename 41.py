...
def strategy_trix(candles, period=15):
    ...
    for i in range(len(closes) - period + 1):
        ema1_list.append(single_ema(closes[i:i+period], period))

    ema2_list = []
    for i in range(len(ema1_list) - period + 1):
        ema2_list.append(single_ema(ema1_list[i:i+period], period))

    ema3_list = []
    for i in range(len(ema2_list) - period + 1):
        ema3_list.append(single_ema(ema2_list[i:i+period], period))

    if len(ema3_list) < 2:
        return "⚪️neutral"

    trix = 100 * (ema3_list[-1] - ema3_list[-2]) / (ema3_list[-2] + 1e-9)
    return "🟢up" if trix > 0 else "🔴down" if trix < 0 else "⚪️neutral"

# Fixed strategy_awesome_oscillator
def strategy_awesome_oscillator(candles):
    if len(candles) < 34:
        return "⚪️neutral"
    midpoints = [(c["high"] + c["low"]) / 2 for c in candles[-34:]]
    short_ema = ema(midpoints[-5:], 5)
    long_ema = ema(midpoints, 34)

    prev_short_ema = ema(midpoints[-6:-1], 5)
    prev_long_ema = ema(midpoints[:-1], 34)
...
def strategy_williams_r(candles, period=14):
    ...
    lows = [c["low"] for c in candles[-period:]]
    current = candles[-1]["close"]
    wr = (max(highs) - current) / (max(highs) - min(lows) + 1e-9) * -100
...
def strategy_volume_spike(candles, period=10):
    ...
    avg = sum(vol[:-1]) / len(vol[:-1]) if len(vol) > 1 else 0
    if vol[-1] > avg * 1.8:
...
def strategy_volume_profile(candles, levels=4):
    price_ranges = []
    lows_50 = [c["low"] for c in candles[-50:]]
    highs_50 = [c["high"] for c in candles[-50:]]
    price_step = (max(highs_50) - min(lows_50)) / levels
    for i in range(levels):
        low = min(lows_50) + i * price_step
        high = low + price_step
...
def strategy_three_white_soldiers(candles):
    ...
    if (first["close"] > first["open"] and
        ...
        third["close"] > third["open"] and
        first["close"] > first["open"] * 1.01 and
...
def strategy_three_black_crows(candles):
    ...
    if (first["close"] < first["open"] and
        second["close"] < second["open"] and
        third["close"] < third["open"] and
        first["close"] < first["open"] * 0.99 and
...
def strategy_volume_weighted_ma(candles, period=20):
    ...
    vwma = vwma_numerator / vwma_denominator if vwma_denominator != 0 else 0
